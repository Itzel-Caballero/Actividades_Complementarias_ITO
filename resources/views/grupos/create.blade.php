    ...
    <div class="section-body">
        <div class="row">
            <div class="col-lg-10 offset-lg-1">
                ...
                <form action="{{ route('grupos.store') }}" method="POST" id="form-grupo">
                    @csrf

                    {{-- ── Datos Generales (compactos en una sola card) ── --}}
                    <div class="card">
                        <div class="card-header">
                            <h4><i class="fas fa-layer-group mr-2 text-primary"></i>Crear Grupo</h4>
                        </div>
                        <div class="card-body">

                            {{-- Fila 1: Actividad | Clave | Semestre --}}
                            <div class="row">
                                <div class="col-md-6">
                                        ...
                                        <select name="id_instructor" class="form-control">
                                            ...
                                            @foreach ($instructores as $inst)
                                                <option value="{{ $inst->id_instructor }}"
                                                    ...
                                                    @if($inst->especialidad) — {{ $inst->especialidad }}@endif
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select name="id_ubicacion" class="form-control">
                                            ...
                                            @foreach ($ubicaciones as $ubic)
                                                <option value="{{ $ubic->id_ubicacion }}"
                                                    ...
                                                    {{ $ubic->espacio }}@if($ubic->capacidad) (cap. {{ $ubic->capacidad }})@endif
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            {{-- Fila 3: Modalidad | Cupo | Estatus | Fecha Inicio | Fecha Fin --}}
                            <div class="row">
                                <div class="col-md-2">
                                    <div class="form-group">
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <label>Cupo Máx. <span class="text-danger">*</span></label>
...
<script>
var DIAS = [
    { id: {{ $diasSemana->where('nombre_dia','lunes')->first()->id_dia    ?? 1 }}, nombre: 'Lunes'     },
    { id: {{ $diasSemana->where('nombre_dia','martes')->first()->id_dia   ?? 2 }}, nombre: 'Martes'    },
    { id: {{ $diasSemana->where('nombre_dia','miercoles')->first()->id_dia ?? 3 }}, nombre: 'Miércoles' },
    { id: {{ $diasSemana->where('nombre_dia','jueves')->first()->id_dia   ?? 4 }}, nombre: 'Jueves'    },
    { id: {{ $diasSemana->where('nombre_dia','viernes')->first()->id_dia  ?? 5 }}, nombre: 'Viernes'   },
    { id: {{ $diasSemana->where('nombre_dia','sabado')->first()->id_dia   ?? 6 }}, nombre: 'Sábado'    }
];
...
@if(old('horarios'))
var oldHorarios = {!! json_encode(old('horarios')) !!};
oldHorarios.forEach(function(h) {
    var diaId = parseInt(h.id_dia);
    var di = -1;
    for (var i = 0; i < DIAS.length; i++) { if (DIAS[i].id === diaId) { di = i; break; } }
    var horaInicio = h.hora_inicio ? h.hora_inicio.substring(0, 5) : '';
    var ti = TIMES.indexOf(horaInicio);
